...
for epoch in range(opt.nepoch):
    ...
    for i, data in enumerate(tqdm(train_dataloader, 0)):
        ...
        inputs_level1, inputs_level1_center = Variable(
            inputs_level1, requires_grad=False), Variable(inputs_level1_center, requires_grad=False)
        ...
        outputs_xyz = train_data.PCA_mean.expand(
            estimation.data.size(0), train_data.PCA_mean.size(1))
        outputs_xyz = torch.addmm(
            outputs_xyz, estimation.data, train_data.PCA_coeff)
    ...
    for i, data in enumerate(tqdm(test_dataloader, 0)):
        ...
        inputs_level1, inputs_level1_center = group_points(points, opt)
        inputs_level1, inputs_level1_center = Variable(
            inputs_level1, volatile=True), Variable(inputs_level1_center, volatile=True)
        ...
        outputs_xyz = test_data.PCA_mean.expand(
            estimation.data.size(0), test_data.PCA_mean.size(1))
        outputs_xyz = torch.addmm(
            outputs_xyz, estimation.data, test_data.PCA_coeff)
        diff = torch.pow(outputs_xyz-gt_xyz, 2).view(-1, opt.JOINT_NUM, 3)
        diff_sum = torch.sum(diff, 2)
        diff_sum_sqrt = torch.sqrt(diff_sum)
        diff_mean = torch.mean(diff_sum_sqrt, 1).view(-1, 1)
